                            ...
                            <td key={col.key}
                              ...
                              {col.key === 'symbol' && flagMap[row.symbol] && (
                                <span className="ml-1 px-1 py-0 bg-orange-900/50 border border-orange-600 rounded text-[8px] text-orange-300 font-bold cursor-help"
                                  data-tip-html={`<div class="rt-hdr">🚩 Flag Pattern — Stock Bee Setup</div>`
                                    + `<div class="rt-row"><div><span class="rt-badge bg-orange">Pole</span></div><div><div class="rt-desc">+${flagMap[row.symbol].poleGain.toFixed(0)}% surge in 1-5 days on heavy volume — the thrust move that started it all</div></div></div>`
                                    + `<div class="rt-row"><div><span class="rt-badge bg-yellow">Flag</span></div><div><div class="rt-desc">${flagMap[row.symbol].flagDays} days of tight consolidation — sellers exhausted, buyers accumulating quietly</div></div></div>`
                                    + `<div class="rt-row"><div><span class="rt-badge bg-emerald">Breakout</span></div><div><div class="rt-desc">Price just broke above the flag high with volume — continuation move starting NOW</div></div></div>`
                                    + `<div class="rt-row"><div><span class="rt-badge bg-neon">Target</span></div><div><div class="rt-desc">Measured move target: Rs.${flagMap[row.symbol].measuredTarget.toFixed(0)} (pole length added to breakout)</div><div class="rt-hit hit-green">71.8% hit rate · R:R 2.51 · Double conviction setup</div></div></div>`}>🚩FLAG</span>
                              )}
                              {col.key === 'symbol' && guppyCoilMap[row.symbol] && (
                                <span className="ml-1 px-1 py-0 bg-cyan-900/50 border border-cyan-500 rounded text-[8px] text-cyan-300 font-bold cursor-help"
                                  data-tip-html={`<div class="rt-hdr">💎 Guppy Coiled — Maximum Compression Breakout</div>`
                                    + `<div class="rt-row"><div><span class="rt-badge bg-cyan">Compression</span></div><div><div class="rt-desc">All 12 Guppy EMAs compressed to just ${guppyCoilMap[row.symbol].avgSpread.toFixed(1)}% spread (min ${guppyCoilMap[row.symbol].minSpread.toFixed(1)}%) during the zone</div></div></div>`
                                    + `<div class="rt-row"><div><span class="rt-badge bg-teal">Energy</span></div><div><div class="rt-desc">Like a spring coiled to its maximum — all moving averages converged, volatility at rock bottom, massive energy stored</div></div></div>`
                                    + `<div class="rt-row"><div><span class="rt-badge bg-neon">Breakout</span></div><div><div class="rt-desc">The coil is now releasing — price breaking out of the compression zone with volume confirmation</div></div></div>`
                                    + `<div class="rt-row"><div><span class="rt-badge bg-emerald">History</span></div><div><div class="rt-desc">Monster moves start here: OSWALAGRO +55%, NATIONALUM +48%, KTKBANK +25%, DHANUKA +55%</div><div class="rt-hit hit-green">Highest conviction setup · Maximum stored energy</div></div></div>`}>💎COILED</span>
                              )}
                              {col.key === 'symbol' && (flagMap[row.symbol] || guppyCoilMap[row.symbol]) && ['BUY','STRONG_BUY','ULTRA_STRONG_BUY'].includes(row.stage) && (
                                <span className="ml-1 text-[9px] cursor-help"
                                  data-tip-html={`<div class="rt-hdr">⚡ Setup Quality Summary — ${row.symbol.replace('.NS','').replace('.BO','')}</div>`
                                    + (guppyCoilMap[row.symbol] ? `<div class="rt-row"><div><span class="rt-badge bg-cyan">💎 COILED</span></div><div><div class="rt-desc">Guppy EMAs compressed to ${guppyCoilMap[row.symbol].avgSpread.toFixed(1)}% — maximum stored energy. When this spring releases, the move is explosive.</div><div class="rt-hit hit-green">Highest conviction overlay</div></div></div>` : '')
                                    + (flagMap[row.symbol] ? `<div class="rt-row"><div><span class="rt-badge bg-orange">🚩 FLAG</span></div><div><div class="rt-desc">Prior +${flagMap[row.symbol].poleGain.toFixed(0)}% pole → ${flagMap[row.symbol].flagDays}d consolidation → breakout. Measured target Rs.${flagMap[row.symbol].measuredTarget.toFixed(0)}</div><div class="rt-hit hit-cyan">R:R 2.51 · Continuation pattern</div></div></div>` : '')
                                    + `<div class="rt-row"><div><span class="rt-badge bg-neon">ACTION</span></div><div><div class="rt-desc">${guppyCoilMap[row.symbol] && flagMap[row.symbol] ? 'RARE DOUBLE SETUP: Both Guppy coil + Flag detected. This is the highest conviction signal possible — prioritize this trade above all others.' : guppyCoilMap[row.symbol] ? 'Guppy coil breakout — expect above-average move size. Position with confidence.' : 'Flag continuation — use measured move target for exit planning.'}</div></div></div>`}>⚡</span>
                              )}
                              {col.key === 'symbol' && (() => {
