...
export const NSE_SYMBOL_ALIASES: SymbolAlias[] = [
...
const ALIAS_MAP = new Map(NSE_SYMBOL_ALIASES.map(a => [a.from, a.to]));

export function splitExchangeSuffix(symbol: string): { base: string; suffix: string } {
  const clean = symbol.trim().toUpperCase();
  const match = clean.match(/^(.*?)(\.(?:NS|BO))$/);
  if (!match) return { base: clean, suffix: '' };
  return { base: match[1], suffix: match[2] };
}

export function canonicalNseSymbolBase(symbol: string): string {
...
export function canonicalNseSymbol(symbol: string): string {
  const { base, suffix } = splitExchangeSuffix(symbol);
  if (!base || base.startsWith('^')) return symbol.trim().toUpperCase();
  return `${canonicalNseSymbolBase(base)}${suffix}`;
}

export function getSymbolAlias(symbol: string): SymbolAlias | undefined {
  const base = splitExchangeSuffix(symbol).base;
  return NSE_SYMBOL_ALIASES.find(a => a.from === base);
}
