...
export interface AnalysisResult {
  ...
  // Hit-Rate Gate: precision tier derived from optimizer-tuned indicator gates
  // 'PREMIUM'  — signal passes all gates; empirical OOS hit-rate ≥70%
  // 'STANDARD' — signal fires but doesn't clear all gates
  // null       — no gate defined for this archetype (ORS-Prime / PerfectStorm)
  ...
  if (n >= 30 && enrich) {
    ...
    try {
      ...
      if (paramSetKey === 'optimized_deployable_20plus') {
        ...
        result.hitRateGate = (
          cmf20v >= 0 && obvV >= 0 && rsi14v >= 45 && result.atrPct14 >= 3
        ) ? 'PREMIUM' : 'STANDARD';
        ...
        result.hitRateGate = (
          cmf20e >= 0.12 && vol >= 1.5 && result.atrPct14 >= 3.0 && rsi2v <= 70
        ) ? 'PREMIUM' : 'STANDARD';
