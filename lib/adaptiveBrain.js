// ═══════════════════════════════════════════════════════════════════════════
// ADAPTIVE BRAIN v2 — Bayesian learning engine for Dr KKR Quant Terminal
// Learns from YOUR closed trades. Gets smarter every day.
// ═══════════════════════════════════════════════════════════════════════════

// Compute brain insights from closed trades
export function computeBrainInsights(trades) {
  ...
  // ── LAYER 1: Individual feature learning (Bayesian) ──
  function bayesianWR(featureFn) {
    ...
    const wr = winsWithFeature.length / withFeature.length;
    return { wr, count: withFeature.length, significant: withFeature.length >= 3 };
  }

  // Apply recency weighting (recent trades weighted 3x)
  function recencyWeightedWR(featureFn) {
    ...
    for (const t of withFeature) {
      const age = (now - new Date(t.closedDate || t.entryDate || '2024-01-01').getTime()) / (86400000 * 30); // months
      const weight = age < 1 ? 3 : age < 3 ? 2 : age < 6 ? 1.5 : 1;
      const isWin = t.status === 'hit_t1' || t.status === 'hit_t2' || t.status === 'hit_t3';
  ...
  const convictions = closed.map(t => ({ conv: t.conviction || 0, win: t.status?.includes('hit') }));
  ...
  // Stock memory (per-stock outcomes)
  const stockMemory = {};
  for (const t of closed) {
    const sym = t.symbol;
  ...
  // Pattern scorecard (candle patterns from closed trades)
  const patternStats = {};
  for (const t of closed) {
  ...
  // Regime performance from closed trades
  const regimeStats = {};
  for (const t of closed) {
  ...
  // MFE/MAE analysis from your trades
  ...
  // ── LAYER 2: Feature interaction discovery ──
  const interactions = [];
  // Only compute if enough trades
  if (closed.length >= 10) {
    const features = [
    ...
    for (let a = 0; a < features.length; a++) {
      for (let b = a + 1; b < features.length; b++) {
        ...
        if (comboWR > baseWR + 0.15 || comboWR < baseWR - 0.15) {
          interactions.push({
            name: `${features[a].name} + ${features[b].name}`,
            trades: combo.length, wins: comboWins, wr: Math.round(comboWR * 100),
            type: comboWR > baseWR ? 'GOLDEN' : 'WEAK'
          });
        }
      }
    }
  ...
  // ── UPGRADE 1: DECAY DETECTION — patterns that stopped working ──
  const decayAlerts = [];
  if (closed.length >= 10) {
  ...
  // ── UPGRADE 2: EMOTIONAL PATTERN DETECTION — revenge trading ──
  let emotionalAlert = null;
  if (closed.length >= 6) {
    // After 2 losses, check if next entry had lower conviction
    for (let i = 2; i < closed.length; i++) {
      const prev2 = [closed[i - 2], closed[i - 1]];
      const bothLoss = prev2.every(t => t.status === 'stopped' || t.status === 'expired');
      if (bothLoss) {
        const afterLossConv = closed[i].conviction || 0;
        const normalConv = winConvAvg;
        if (afterLossConv < normalConv - 15 && !(closed[i].status?.includes('hit'))) {
          emotionalAlert = {
            pattern: 'revenge_trade',
            message: `After losses, you entered at conviction ${Math.round(afterLossConv)} (your avg winner: ${Math.round(normalConv)}). These low-conviction post-loss entries tend to fail.`,
            advice: 'After 2 losses, wait for a signal with conviction ≥ ' + Math.round(normalConv)
          };
        }
      }
    }
  ...
  // ── UPGRADE 3: OPTIMAL HOLDING PERIOD ──
  const holdingInsight = { avgHold: 0, optimalHold: '', t2CaptureRate: 0, advice: '' };
  if (wins.length >= 3) {
    ...
    if (holdingInsight.t2CaptureRate < 40) {
      holdingInsight.advice = `Only ${holdingInsight.t2CaptureRate}% of your T1 hits continue to T2. Consider holding 50% position 2 days longer.`;
    } else {
      holdingInsight.advice = `${holdingInsight.t2CaptureRate}% T2 capture — your partial exit model is working well.`;
    }
  }

  // ── UPGRADE 4: DAY-OF-WEEK LEARNING ──
  ...
  // ── UPGRADE 5: CONFIDENCE INTERVALS ──
  function confidenceInterval(score, tradeCount) {
    // Wilson interval width based on sample size
    // Fewer trades = wider interval = less certainty
  ...
  // ── LAYER 6: Dynamic position sizing ──
  ...
  // ── Compute brain-adjusted score for a signal ──
  function adjustScore(signal, extraData) {
    ...
    if (secData && secData.total >= 2) {
      const secWR = secData.wins / secData.total;
      const adj = Math.round((secWR - baseWR) * 30);
      score += adj;
      if (Math.abs(adj) >= 3) adjustments.push({ factor: `Sector ${sec}`, adj, reason: `${Math.round(secWR*100)}% WR for you` });
    }

    // Stock memory adjustment
    const mem = stockMemory[signal.symbol];
    if (mem) {
      if (mem.consecutiveStops >= 2) { score -= 15; adjustments.push({ factor: 'Stock CAUTION', adj: -15, reason: `${mem.consecutiveStops} consecutive stops` }); }
      else if (mem.wins >= 2 && mem.stops === 0) { score += 8; adjustments.push({ factor: 'Stock RELIABLE', adj: +8, reason: `${mem.wins} wins, 0 stops` }); }
    }

    // Streak adjustment
    if (streakType === 'L' && currentStreak >= 2) { score -= 5; adjustments.push({ factor: 'Cold streak', adj: -5, reason: `${currentStreak} consecutive losses` }); }
    if (streakType === 'W' && currentStreak >= 3) { score += 3; adjustments.push({ factor: 'Hot streak', adj: +3, reason: `${currentStreak} consecutive wins` }); }

    // Conviction vs learned threshold
    if (closed.length >= 5 && signal.conviction < optimalConviction - 10) {
      const adj = -Math.round((optimalConviction - signal.conviction) * 0.3);
      score += adj;
      adjustments.push({ factor: 'Below your threshold', adj, reason: `Your winners avg ${Math.round(winConvAvg)} conviction` });
    }

    // Clenow score (from extraData)
    if (extraData?.clenowScore != null) {
      if (extraData.clenowScore >= 80) { score += 5; adjustments.push({ factor: 'Clenow strong', adj: +5, reason: `Score ${extraData.clenowScore.toFixed(0)}, smooth trend` }); }
      else if (extraData.clenowScore < 0) { score -= 5; adjustments.push({ factor: 'Clenow negative', adj: -5, reason: `Score ${extraData.clenowScore.toFixed(0)}, no trend` }); }
    }

    // Flag/Coiled overlay bonus
    if (extraData?.hasFlag) { score += 5; adjustments.push({ factor: '🚩 Flag pattern', adj: +5, reason: 'Double conviction setup' }); }
    if (extraData?.hasCoiled) { score += 5; adjustments.push({ factor: '💎 Guppy coiled', adj: +5, reason: 'Maximum stored energy' }); }

    // Decay detection — reduce score for decaying sectors
    for (const da of decayAlerts) {
      if (da.type === 'sector' && (signal.sector || extraData?.sector) === da.name) {
        score -= 8; adjustments.push({ factor: `Sector decay: ${da.name}`, adj: -8, reason: `Was ${da.oldWR}% WR → now ${da.newWR}% (rotated)` });
      }
    }

    // Emotional pattern — warn if this looks like a revenge trade
    if (emotionalAlert && streakType === 'L' && currentStreak >= 2 && (signal.conviction || 0) < winConvAvg - 10) {
      score -= 10; adjustments.push({ factor: 'Revenge trade risk', adj: -10, reason: emotionalAlert.advice });
    }

    // Day-of-week adjustment
    if (worstDay && worstDay.wr < baseWR * 100 - 20) {
      const today = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][new Date().getDay()];
      if (today === worstDay.day) { score -= 5; adjustments.push({ factor: `${today} is your worst day`, adj: -5, reason: `${worstDay.wr}% WR on ${today}s` }); }
    }
    if (bestDay && bestDay.wr > baseWR * 100 + 15) {
      const today = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][new Date().getDay()];
      if (today === bestDay.day) { score += 3; adjustments.push({ factor: `${today} is your best day`, adj: +3, reason: `${bestDay.wr}% WR on ${today}s` }); }
    }
    ...
    return { originalScore: signal.conviction || 50, brainScore: score, adjustments, sizing, confidence, confidenceInterval: ci };
  }
  ...
  function premortem(signal, extraData) {
    ...
    return {
      matches: similar.map(t => ({
        symbol: t.symbol, conviction: t.conviction, status: t.status,
        pnlPct: t.pnlPct, similarity: t.similarity
      })),
  ...
  return {
    ...
    // Upgrade 3: Holding
    holdingInsight,
    // Upgrade 4: Day-of-week
    dayScorecard, bestDay, worstDay,
    // Upgrade 5: Confidence intervals
    confidenceInterval,
    // Functions
