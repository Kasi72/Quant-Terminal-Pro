...
// Engine 2: Thompson Sampling (signal prioritization) — multi-factor
...
export function computeBrainInsights(trades, nseContext = null) {
  ...
  for (const t of closed) {
    ...
    else if (t.status === 'stopped') patternStats[pat].losses++;
  }
  ...
  // Used by multi-factor Thompson Sampling to factor in setup-type context.
  ...
  for (const t of closed) {
    ...
    if (t.status?.includes('hit')) atrStateMemory[atr].wins++;
  }

  // Conviction tier memory — LOW (<45), MEDIUM (45–70), HIGH (>70)
  // Separating tiers lets Thompson distinguish overconfident entries (low tier, poor WR)
  ...
  for (const t of closed) {
    ...
    if (t.status?.includes('hit')) convTierMemory[tier].wins++;
  }

  // Streak detection
  // Only count decided trades (wins/losses) in streak — expired don't break or extend streaks
  const decidedRecent = closed.slice(-20).filter(t => t.status?.includes('hit') || t.status === 'stopped').slice(-10);
  const recentOutcomes = decidedRecent.map(t => t.status?.includes('hit') ? 'W' : 'L');
  ...
  for (const t of closed) {
    ...
    else if (t.status === 'stopped') regimeStats[reg].losses++;
  }
  ...
  // Expectancy = avg win × WR − avg loss × (1−WR) — the single most honest system stat
  const expectancy = totalDecided > 0 ? Math.round(((baseWR * avgWinPnl) - ((1 - baseWR) * avgLossPnl)) * 100) / 100 : 0;
  ...
  if (closed.length >= 10) {
    ...
    for (let a = 0; a < features.length; a++) {
      for (let b = a + 1; b < features.length; b++) {
        ...
        const comboDecided = combo.filter(t => t.status?.includes('hit') || t.status === 'stopped').length;
  ...
  for (const t of closed) {
    ...
    else if (t.status === 'stopped') dayOfWeekStats[day].losses++;
  }
  ...
  function thompsonRank(signals, extraDataMap) {
    ...
    const ranked = signals.map(signal => {
      ...
      return { symbol: signal.symbol, thompsonScore: Math.round(thompsonSample * 100), alpha, beta, expectedWR: Math.round(alpha / (alpha + beta) * 100), factorSummary };
    });
  ...
  function adjustScore(signal, extraData) {
    ...
    const ci = confidenceInterval(baseWR, closed.length);
  ...
  function analyzeExitBehavior() {
    ...
    const t2Hits = closed.filter(t => t.status === 'hit_t2' || t.status === 'hit_t3').length;
    const t1OnlyHits = closed.filter(t => t.status === 'hit_t1').length;
    const expiredCount = closed.filter(t => t.status === 'expired').length;
    const reachedT1 = t1OnlyHits + t2Hits;
    const t2CaptureRate = reachedT1 > 0 ? Math.round(t2Hits / reachedT1 * 100) : 0;
    const expireRate = closed.length > 0 ? Math.round(expiredCount / closed.length * 100) : 0;

    // MFE vs actual exit P&L gap — how much you left on the table per winning trade
    const winsWithMFE = wins.filter(t => t.mfe != null && t.pnlPct != null && (t.mfe || 0) > 0);
    const avgMFEGap = winsWithMFE.length > 0
      ? winsWithMFE.reduce((s, t) => s + (Math.abs(t.mfe || 0) - Math.abs(t.pnlPct || 0)), 0) / winsWithMFE.length
      : 0;
  ...
  return {
    ...
    // Multi-factor Thompson internals (exposed for debugging/UI)
...
export function getSetupQuality(prior, stage, paramSetKey) {
  ...
  // Cascade: combo → stage-only → param-only → global (Bug 17 fix: paramOnly was computed but never used)
  const ref = (combo?.n >= 3 ? combo : null) || (stageOnly?.n >= 3 ? stageOnly : null) || (paramOnly?.n >= 3 ? paramOnly : null);
  if (!ref) return null;
...
// When the live brain has few trades (LOW confidence), the 719-trade backtest prior
