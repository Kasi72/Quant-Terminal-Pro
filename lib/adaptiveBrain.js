...
// Engine 2: Deterministic Beta posterior ranking (signal prioritization)
...
// Engine 6: NSE Calendar Intelligence (expiry week, budget week, FII flows)
// ═══════════════════════════════════════════════════════════════════════════
...
// ── NSE Market Calendar ─────────────────────────────────────────────────────
// Structural NSE calendar events that distort breakout reliability:
// (1) F&O expiry week — last Thursday of each month ± 4 trading days.
//     IV crush and hedge unwinding generate false breakout signals.
// (2) Union Budget week — Feb 1 each year ± 3 days. Binary outcomes.
// (3) FII sell streak — user-supplied; institutional headwind compresses upside.

function _lastThursdayOfMonth(year, month) {
  const lastDay = new Date(year, month + 1, 0); // last calendar day of month
  while (lastDay.getDay() !== 4) lastDay.setDate(lastDay.getDate() - 1); // walk back to Thursday
  return lastDay;
}

export function getNSECalendarContext(fiiSellStreak = 0) {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const y = today.getFullYear(), m = today.getMonth();
  const expiry = _lastThursdayOfMonth(y, m);
  const daysToExpiry = Math.round((expiry.getTime() - today.getTime()) / 86400000);
  // Bug 18 fix: calendar-day window must be wider than 4 to survive weekends (Mon = 7 cal days before Thu)
  const isExpiryWeek = daysToExpiry >= 0 && daysToExpiry <= 7;
  // Budget = Feb 1 ± 3 calendar days
  const budgetDay = new Date(y, 1, 1);
  const isBudgetWeek = Math.abs(today.getTime() - budgetDay.getTime()) / 86400000 <= 3;
  return {
    isExpiryWeek,
    isBudgetWeek,
    fiiSellStreak: Math.max(0, fiiSellStreak | 0),
    daysToExpiry: isExpiryWeek ? daysToExpiry : -1,
  };
}

export function computeBrainInsights(trades, nseContext = null) {
  // Brain uses the same validation objective as the Validation tab:
  // a decided trade is either a +5% MFE hit, or a terminal no-hit.
  ...
  const totalDecided = closed.length;
  ...
  const isWin = didReachFivePctTarget;
  const isLoss = t => !didReachFivePctTarget(t);
  const outcomePct = t => isWin(t) ? Math.max(t.pnlPct ?? 0, getTradeMfePct(t)) : (t.pnlPct ?? 0);
  ...
  function bayesianWR(featureFn) {
    ...
    const winsWithFeature = withFeature.filter(isWin);
    if (withFeature.length < 3) return { wr: baseWR, count: 0, significant: false };
  ...
  function recencyWeightedWR(featureFn) {
    ...
    for (const t of withFeature) {
      ...
      if (isWin(t)) weightedWins += weight;
      totalWeight += weight;
    }
  ...
  // Threshold = just below your winners' average (not midpoint of wins/losses,
  // which drifts above the winner mean when the gap is small — penalising good entries)
  const optimalConviction = Math.round(winConvAvg - 5);
  ...
  for (const t of closed) {
    ...
    if (isWin(t)) sectorStats[sec].wins++;
    else sectorStats[sec].losses++;
  }
  const sectorScorecard = Object.entries(sectorStats).map(([sector, s]) => {
    ...
    return {
      sector, ...s, wr,
      status: decided >= 2 ? (wr >= 70 ? 'HOT' : wr >= 40 ? 'NEUTRAL' : 'COLD') : 'INSUFFICIENT'
    };
  }).sort((a, b) => b.wr - a.wr);

  // Stock memory
  const stockMemory = {};
  const closedByDate = [...closed].sort((a, b) => (a.closedDate || a.entryDate || '').localeCompare(b.closedDate || b.entryDate || ''));
  for (const t of closedByDate) {
    ...
    if (isWin(t)) { m.wins++; m.consecutiveStops = 0; m.lastOutcome = 'win'; }
    else { m.stops++; m.consecutiveStops++; if (t.status === 'expired') m.expired++; m.lastOutcome = t.status === 'expired' ? 'expired' : 'no_hit'; }
  }
  ...
  for (const t of closed) {
    ...
    if (!patternStats[pat]) patternStats[pat] = { wins: 0, losses: 0, total: 0 };
    patternStats[pat].total++;
    if (isWin(t)) patternStats[pat].wins++;
    else patternStats[pat].losses++;
  }
  const patternScorecard = Object.entries(patternStats)
    .filter(([, s]) => s.total >= 2)
    .map(([pattern, s]) => {
      const decided = s.wins + s.losses;
      return { pattern, ...s, wr: decided > 0 ? Math.round(s.wins / decided * 100) : 0 };
    })
    .sort((a, b) => b.wr - a.wr);
  const bestPattern = patternScorecard[0] || null;
  const worstPattern = patternScorecard[patternScorecard.length - 1] || null;

  // ATR state memory — wins/losses per ATR state (NORMAL, EXPLOSION, SQUEEZE, INFLECT…)
  // Used by multi-factor posterior ranking to factor in setup-type context.
  const atrStateMemory = {};
  for (const t of closed) {
    const atr = t.atrState || 'UNKNOWN';
    if (!atrStateMemory[atr]) atrStateMemory[atr] = { wins: 0, total: 0 };
    atrStateMemory[atr].total++;
    if (isWin(t)) atrStateMemory[atr].wins++;
  }

  // Conviction tier memory — LOW (<45), MEDIUM (45–70), HIGH (>70)
  // Separating tiers lets posterior ranking distinguish overconfident entries (low tier, poor WR)
  // from high-conviction setups that warrant more weight.
  function getConvTier(c) { return c >= 70 ? 'HIGH' : c >= 45 ? 'MEDIUM' : 'LOW'; }
  const convTierMemory = { HIGH: { wins: 0, total: 0 }, MEDIUM: { wins: 0, total: 0 }, LOW: { wins: 0, total: 0 } };
  for (const t of closed) {
    const tier = getConvTier(t.conviction || 0);
    convTierMemory[tier].total++;
    if (isWin(t)) convTierMemory[tier].wins++;
  }

  // Streak detection
  // Only count +5%-objective decided trades in streak.
  const decidedRecent = closed.slice(-10);
  const recentOutcomes = decidedRecent.map(t => isWin(t) ? 'W' : 'L');
  ...
  for (const t of closed) {
    ...
    if (isWin(t)) regimeStats[reg].wins++;
  ...
  const regimeScorecard = Object.entries(regimeStats)
    ...
    .map(([regime, s]) => {
      const decided = s.wins + s.losses;
      return { regime, ...s, wr: decided > 0 ? Math.round(s.wins / decided * 100) : 0 };
    })
    .sort((a, b) => b.wr - a.wr);
  ...
  const avgMFE = wins.length > 0 ? wins.reduce((s, t) => s + getTradeMfePct(t), 0) / wins.length : 0;
  const avgMAE = closed.length > 0 ? closed.reduce((s, t) => s + getTradeMaePct(t), 0) / closed.length : 0;
  const avgWinPnl = wins.length > 0 ? wins.reduce((s, t) => s + outcomePct(t), 0) / wins.length : 0;
  const avgLossPnl = losses.length > 0 ? losses.reduce((s, t) => s + outcomePct(t), 0) / losses.length : 0;
  // Expectancy = average signed objective outcome. +5% hits use MFE potential;
  // terminal no-hits use realised P&L, which may be positive or negative.
  const expectancy = totalDecided > 0 ? Math.round((closed.reduce((s, t) => s + outcomePct(t), 0) / totalDecided) * 100) / 100 : 0;
  ...
  if (closed.length >= 10) {
    ...
    for (let a = 0; a < features.length; a++) {
      for (let b = a + 1; b < features.length; b++) {
        ...
        const comboWins = combo.filter(isWin).length;
        const comboDecided = combo.length;
        const comboWR = comboDecided > 0 ? comboWins / comboDecided : 0;
  ...
  // invisible.
  const decayAlerts = [];
  const closedByDateDecay = [...closed].sort((a, b) => (a.closedDate || a.entryDate || '').localeCompare(b.closedDate || b.entryDate || ''));
  const recentHalf = closedByDateDecay.length >= 10 ? closedByDateDecay.slice(-Math.floor(closedByDateDecay.length / 2)) : [];
  const olderHalf = closedByDateDecay.length >= 10 ? closedByDateDecay.slice(0, Math.floor(closedByDateDecay.length / 2)) : [];
  function detectDecay(type, groupKeyFn, statsObj, minTotal) {
    ...
    for (const [key, stats] of Object.entries(statsObj)) {
      ...
      const recentWR = recentG.filter(isWin).length / recentG.length;
      const olderWR = olderG.filter(isWin).length / olderG.length;
  ...
  if (closed.length >= 6) {
    for (let i = 2; i < closed.length; i++) {
      ...
      if (prev2.every(isLoss)) {
        const afterLossConv = closed[i].conviction || 0;
        if (afterLossConv < winConvAvg - 15 && !isWin(closed[i])) {
  ...
  if (wins.length >= 3) {
    ...
    const reachedTargets = trades.filter(t => ['hit_t1', 'hit_t2', 'hit_t3'].includes(t.status));
    const t2Hits = reachedTargets.filter(t => t.status === 'hit_t2' || t.status === 'hit_t3').length;
    holdingInsight.t2CaptureRate = reachedTargets.length > 0 ? Math.round(t2Hits / reachedTargets.length * 100) : 0;
  ...
  for (const t of closed) {
    ...
    if (!dayOfWeekStats[day]) dayOfWeekStats[day] = { wins: 0, losses: 0, total: 0 };
    dayOfWeekStats[day].total++;
    if (isWin(t)) dayOfWeekStats[day].wins++;
    else dayOfWeekStats[day].losses++;
  }
  const dayScorecard = Object.entries(dayOfWeekStats)
    .filter(([, s]) => s.total >= 8)
    .map(([day, s]) => {
      const decided = s.wins + s.losses;
      return { day, ...s, wr: decided > 0 ? Math.round(s.wins / decided * 100) : 0 };
    })
    .sort((a, b) => b.wr - a.wr);
  ...
  // sample size where available), falling back to total +5%-decided trades.
  ...
  // Bug 16 fix: Wilson CI is only valid for Bernoulli proportions (win/loss), not composite scores.
  // Apply it to the raw win rate, not the composite score.
  function confidenceInterval(winRate, tradeCount) {
    const n = Math.max(1, tradeCount);
    const { low, high } = wilsonInterval(Math.max(0, Math.min(1, winRate)), n);
    return { low: Math.round(low), high: Math.round(high), width: Math.round(high - low) };
  }

  // ══════════════════════════════════════════════════════════════════
  // ENGINE 2: BETA POSTERIOR RANKING — Signal Prioritization
  // Uses a deterministic Beta posterior score to rank multiple signals
  // by probability of success. This keeps rankings stable across scans
  // while still giving sparse but promising buckets a small uncertainty bonus.
  // ══════════════════════════════════════════════════════════════════

  function thompsonRank(signals, extraDataMap) {
    if (signals.length === 0) return [];
    // Multi-factor Beta prior: 5 independent data sources each contribute
    // pseudo-counts scaled by sqrt(n). Small buckets (2–3 trades) contribute
    // fractional counts; large buckets (10+) contribute full counts, preventing
    // any single factor from dominating when sample sizes are uneven.
    function addFactor(a, b, wins, total, cap = 5) {
      if (total < 2) return [a, b];
      const scale = Math.min(cap, Math.sqrt(total));
      return [a + (wins / total) * scale, b + ((total - wins) / total) * scale];
    }
    const ranked = signals.map(signal => {
      const extra = extraDataMap?.[signal.symbol] || {};
      const sec = signal.sector || extra.sector || 'Unknown';
      // Uninformative prior: 1 pseudo-win, 1 pseudo-loss
      let alpha = 1, beta = 1;
      // Factor 1: Sector history
      const secData = sectorStats[sec];
      if (secData) [alpha, beta] = addFactor(alpha, beta, secData.wins, secData.total);
      // Factor 2: Stock-specific memory (only decided trades: wins + stops)
      const mem = stockMemory[signal.symbol];
      const memTotal = mem ? mem.wins + mem.stops : 0;
      if (mem && memTotal >= 1) [alpha, beta] = addFactor(alpha, beta, mem.wins, memTotal);
      // Factor 3: ATR state history (NORMAL/EXPLOSION/SQUEEZE/INFLECT…)
      const atr = extra.atrState || signal.atrState;
      if (atr) { const d = atrStateMemory[atr]; if (d) [alpha, beta] = addFactor(alpha, beta, d.wins, d.total); }
      // Factor 4: Candle pattern history
      const pat = extra.candlePattern || signal.stats?.candlePattern;
      if (pat) { const d = patternStats[pat]; if (d) [alpha, beta] = addFactor(alpha, beta, d.wins, d.total); }
      // Factor 5: Conviction tier history (LOW / MEDIUM / HIGH)
      const conv = extra.conviction ?? signal.conviction ?? 0;
      const tierD = convTierMemory[getConvTier(conv)];
      if (tierD) [alpha, beta] = addFactor(alpha, beta, tierD.wins, tierD.total);

      const posteriorMean = alpha / (alpha + beta);
      const posteriorSd = Math.sqrt((alpha * beta) / (((alpha + beta) ** 2) * (alpha + beta + 1)));
      const posteriorScore = Math.min(0.99, posteriorMean + 0.5 * posteriorSd);
      const factorSummary = [
        secData?.total >= 2 ? `${sec}:${Math.round(secData.wins / secData.total * 100)}%` : null,
        mem && memTotal >= 1 ? `${signal.symbol}:${Math.round(mem.wins / memTotal * 100)}%` : null,
        atr && atrStateMemory[atr]?.total >= 2 ? `ATR-${atr}:${Math.round(atrStateMemory[atr].wins / atrStateMemory[atr].total * 100)}%` : null,
        pat && patternStats[pat]?.total >= 2 ? `${pat}:${Math.round(patternStats[pat].wins / patternStats[pat].total * 100)}%` : null,
        tierD?.total >= 2 ? `Conv-${getConvTier(conv)}:${Math.round(tierD.wins / tierD.total * 100)}%` : null,
      ].filter(Boolean).join(', ');

      return { symbol: signal.symbol, thompsonScore: Math.round(posteriorScore * 100), alpha, beta, expectedWR: Math.round(posteriorMean * 100), factorSummary };
    });
  ...
  // Build winner profile from 90-day rolling window (falls back to all-time if < 3 recent)
  // All-time profiles go stale — your winning sector mix two years ago may no longer apply.
  const ninetyDaysAgo = Date.now() - 90 * 86400000;
  const recentWins = wins.filter(t => new Date(t.closedDate || t.entryDate || '2024-01-01').getTime() >= ninetyDaysAgo);
  const profileWins = recentWins.length >= 3 ? recentWins : wins;
  const winnerProfile = { conviction: { min: 100, max: 0, mean: 0 }, count: 0, isRolling: recentWins.length >= 3 };
  const winnerSectors = new Set();
  const winnerAtrStates = new Set();
  const winnerPatterns = new Set();
  if (profileWins.length >= 3) {
    winnerProfile.count = profileWins.length;
    const convs = profileWins.map(t => t.conviction || 0).filter(c => c > 0);
    if (convs.length > 0) {
  ...
  if (closed.length >= 3) {
    ...
    if (closed.length >= 10) {
      ...
      const recentWR = recent5.filter(isWin).length / recent5.length;
      const prevWR = prev5.length > 0 ? prev5.filter(isWin).length / prev5.length : 0.5;
  ...
  for (const t of closed) {
    ...
    for (const s of sets) {
      ...
      if (isWin(t)) { paramSetStats[s].wins++; paramSetStats[s].totalReward += outcomePct(t); }
      else { paramSetStats[s].totalReward += (t.pnlPct || -getTradeMaePct(t) || -2); }
    }
  }
  detectDecay('paramSet', t => t.paramSetKey || 'unknown', paramSetStats, 3);

  // UCB1 score for each param set
  const totalPlays = Math.max(2, Object.values(paramSetStats).reduce((s, p) => s + p.total, 0));
  ...
  function adjustScore(signal, extraData) {
    ...
    const secData = sectorStats[sec];
    let ciWinRate = baseWR;
    let ciCount = closed.length;
    if (secData && secData.total >= 2) {
      const secDecided = secData.wins + secData.losses;
      const secWR = secDecided > 0 ? secData.wins / secDecided : baseWR;
      if (secDecided >= 3) { ciWinRate = secWR; ciCount = secDecided; }
      const adj = secDecided > 0 ? Math.round((secWR - baseWR) * 30) : 0;
      score += adj;
      if (Math.abs(adj) >= 3) adjustments.push({ factor: `Sector ${sec}`, adj, reason: `${Math.round(secWR * 100)}% WR for you`, engine: 'Bayesian' });
    }
    const mem = stockMemory[signal.symbol];
    const memTotalForCi = mem ? mem.wins + mem.stops : 0;
    if (memTotalForCi >= 3) { ciWinRate = mem.wins / memTotalForCi; ciCount = memTotalForCi; }
    if (mem) {
    ...
    // ENGINE 6: NSE Calendar Intelligence
    // Structural market events that mechanically compress breakout reliability
    // independent of signal quality. Applied last so they are always visible
    // as distinct adjustment rows — never absorbed into other factors.
    if (nseContext) {
      if (nseContext.isExpiryWeek) {
        const adj = -8;
        score += adj;
        adjustments.push({ factor: '📅 F&O Expiry week', adj, reason: `Expiry in ${nseContext.daysToExpiry}d — IV crush distorts breakouts`, engine: 'Calendar' });
      }
      if (nseContext.isBudgetWeek) {
        const adj = -10;
        score += adj;
        adjustments.push({ factor: '📋 Budget week', adj, reason: 'Binary outcome risk — reduce size 50%', engine: 'Calendar' });
      }
      if (nseContext.fiiSellStreak >= 5) {
        const adj = -8;
        score += adj;
        adjustments.push({ factor: `🔴 FII sell ${nseContext.fiiSellStreak}d`, adj, reason: `Institutional headwind — ${nseContext.fiiSellStreak} consecutive net-sell days`, engine: 'Calendar' });
      } else if (nseContext.fiiSellStreak >= 3) {
        const adj = -4;
        score += adj;
        adjustments.push({ factor: `⚠ FII sell ${nseContext.fiiSellStreak}d`, adj, reason: 'FII net sellers — watch for momentum reversal', engine: 'Calendar' });
      }
    }

    score = Math.max(0, Math.min(100, Math.round(score)));
    const sizing = getRecommendedRisk(score);
    const ci = confidenceInterval(ciWinRate, ciCount);
  ...
  // ══════════════════════════════════════════════════════════════════
  // EXIT BEHAVIOR LEARNING — Are you selling too soon or holding too long?
  // Analyses the gap between where you exited and where T2 was available.
  // ══════════════════════════════════════════════════════════════════
  function analyzeExitBehavior() {
    if (closed.length < 5) return null;
    const t2Hits = trades.filter(t => t.status === 'hit_t2' || t.status === 'hit_t3').length;
    const t1OnlyHits = trades.filter(t => t.status === 'hit_t1').length;
    const expiredCount = terminal.filter(t => t.status === 'expired').length;
    const reachedT1 = t1OnlyHits + t2Hits;
    const t2CaptureRate = reachedT1 > 0 ? Math.round(t2Hits / reachedT1 * 100) : 0;
    const expireRate = terminal.length > 0 ? Math.round(expiredCount / terminal.length * 100) : 0;

    // MFE vs actual exit P&L gap — how much you left on the table per winning trade
    const winsWithMFE = wins.filter(t => getTradeMfePct(t) > 0);
    const avgMFEGap = winsWithMFE.length > 0
      ? winsWithMFE.reduce((s, t) => s + (getTradeMfePct(t) - Math.max(t.pnlPct ?? 0, 0)), 0) / winsWithMFE.length
      : 0;

    let sellerType, advice;
    if (reachedT1 >= 3 && t2CaptureRate < 30) {
      sellerType = 'SELL_TOO_SOON';
      advice = `Only ${t2CaptureRate}% of your T1 hits go on to T2 — you're banking profit early. Hold 40-50% position after T1. Avg ${avgMFEGap.toFixed(1)}% left on table per winner.`;
    } else if (expireRate > 35 && expiredCount >= 3) {
      sellerType = 'HOLD_TOO_LONG';
      advice = `${expireRate}% of trades expire without a target. Book half at T1 and trail the rest — don't give back open profit waiting for T2.`;
    } else if (t2CaptureRate >= 60 && reachedT1 >= 3) {
      sellerType = 'PATIENT_HOLDER';
      advice = `Excellent ${t2CaptureRate}% T2 capture — you let winners run. Watch for regime shifts that may require faster booking.`;
    } else {
      sellerType = 'BALANCED';
      advice = `Balanced exit pattern (${t2CaptureRate}% T2 rate, ${expireRate}% expire). System working as designed.`;
    }
    return { t2CaptureRate, avgMFEGap: Math.round(avgMFEGap * 10) / 10, expireRate, sellerType, advice, t2Hits, t1OnlyHits, reachedT1, expiredCount };
  }
  ...
  function premortem(signal, extraData) {
    ...
    const simWins = similar.filter(isWin).length;
    return {
  ...
  return {
    ...
    avgWinPnl: Math.round(avgWinPnl * 100) / 100, avgLossPnl: Math.round(avgLossPnl * 100) / 100, expectancy,
    decayAlerts, emotionalAlert, holdingInsight,
    dayScorecard, bestDay, worstDay, confidenceInterval,
    analyzeExitBehavior,
    // Engine 2: deterministic Beta posterior ranking
    thompsonRank,
    ...
    // Engine 6: NSE Calendar
    nseCalendar: nseContext,
    // Multi-factor posterior ranking internals (exposed for debugging/UI)
    atrStateMemory, convTierMemory,
  };
}
 ...
 * from the bundled backtest prior.
...
export function getSetupQuality(prior, stage, paramSetKey) {
  ...
  // Cascade: combo → stage-only → param-only. Return the source so UI can
  // disclose when a broad fallback is being used for a sparse exact combo.
  let ref = null, source = 'none';
  if (combo?.n >= 3) { ref = combo; source = 'exact'; }
  else if (stageOnly?.n >= 3) { ref = stageOnly; source = 'stage'; }
  else if (paramOnly?.n >= 3) { ref = paramOnly; source = 'param'; }
  if (!ref) return null;
  ...
  // Tier classification — calibrated to the bundled prior's realised avg P&L range.
  ...
  const fallback = source !== 'exact';
  const label = `${fallback ? '~' : ''}${(expectedPnl >= 0 ? '+' : '')}${expectedPnl.toFixed(2)}% avg · ${wr}% WR · n=${n}${fallback ? ` · ${source} fallback` : ''}`;

  return { expectedPnl, wr, n, medPnl, tier, color, sizeMultiplier, label, combo: source === 'exact', source, fallback, exactN: combo?.n ?? 0 };
}
 ...
 */
// brainConfidence: 'LOW'|'DEVELOPING'|'MODERATE'|'GOOD'|'HIGH'|'EXPERT'
// When the live brain has few trades (LOW confidence), the bundled backtest prior
// is more reliable — lean on it. As live data accumulates, shift weight to brain.
const BRAIN_WEIGHT_BY_CONF = { LOW: 0.2, DEVELOPING: 0.35, MODERATE: 0.5, GOOD: 0.6, HIGH: 0.7, EXPERT: 0.8 };

export function rankSignalsByBrainV2(signals, brainScores, prior, brainConfidence) {
  if (!signals?.length) return [];

  const brainWeight = BRAIN_WEIGHT_BY_CONF[brainConfidence ?? 'LOW'] ?? 0.5;
  const priorWeight = 1 - brainWeight;
  ...
  return signals.map(sig => {
    ...
    const compositeScore = Math.round(brainWeight * brainS + priorWeight * qualityS);
    ...
    return {
      ...
      brainWeight: Math.round(brainWeight * 100),
    };
