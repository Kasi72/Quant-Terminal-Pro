...
export function computeBrainInsights(trades) {
  ...
  const recentHalf = closedByDate.length >= 10 ? closedByDate.slice(-Math.floor(closedByDate.length / 2)) : [];
  const olderHalf = closedByDate.length >= 10 ? closedByDate.slice(0, Math.floor(closedByDate.length / 2)) : [];
