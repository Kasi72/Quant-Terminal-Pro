...
export interface BreakoutSignal {
  isBreakout: boolean;      // did bar i cross the level on this model's rules?
  level: number;            // the resistance level in question (0 if undefined)
  distancePct: number;      // (close - level)/level × 100  (neg = still below)
  meta: Record<string, number | boolean>;
}

// Precomputed rolling series so batch evaluation is O(1) per bar.
// All arrays are indexed by bar; entry [i] uses only bars strictly before i.
export interface SeriesCtx {
  atr14:        number[];
  avgVol20:     number[];
  ema10:        number[];
  ema50:        number[];
  hh252:        number[];              // highest high of [i-252, i-1] (52-week lid)
  maxDownVol10: number[];              // max volume among down-days in [i-10, i-1]
  hh:           Record<number, number[]>; // window N → highest high of [i-N, i-1]
}

const NONE: BreakoutSignal = { isBreakout: false, level: 0, distancePct: 0, meta: {} };

// ── ctx builders (used by the harness; safe to call standalone) ───────────────

export function buildCtx(candles: Candle[], hhWindows: number[]): SeriesCtx {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (i < 14) { trSum += tr; if (i === 13) atr = trSum / 14; }
  ...
  for (let i = 0; i < n; i++) {
    if (i < 20) { avgVol20[i] = 0; continue; }
    let s = 0;
    for (let j = i - 20; j < i; j++) s += candles[j].v;
    avgVol20[i] = s / 20;
  }
  ...
  for (let i = 0; i < n; i++) {
    ...
    let m = 0;
    for (let j = from; j < i; j++) if (candles[j].h > m) m = candles[j].h;
    hh252[i] = m;
  }
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = Math.max(1, i - 10); j < i; j++) {
      if (candles[j].c < candles[j - 1].c && candles[j].v > m) m = candles[j].v;
    }
    maxDownVol10[i] = m;
  }
...
function hhLocal(candles: Candle[], i: number, N: number): number {
  let m = 0;
  for (let j = Math.max(0, i - N); j < i; j++) if (candles[j].h > m) m = candles[j].h;
  return m;
}
function avgVolLocal(candles: Candle[], i: number, N = 20): number {
  if (i < N) return 0;
  let s = 0;
  for (let j = i - N; j < i; j++) s += candles[j].v;
  return s / N;
}

// ── Model A — Donchian N-day high (pure objective channel) ───────────────────

export interface DonchianParams { N: number; volMult: number; }
export function modelDonchian(candles: Candle[], i: number, p: DonchianParams, ctx?: SeriesCtx): BreakoutSignal {
  if (i < 1) return NONE;
...
// ── Model B — Swing-pivot breakout (structure-based) ─────────────────────────

export interface SwingParams { k: number; lookback: number; volMult: number; }
...
export function modelSwing(candles: Candle[], i: number, p: SwingParams, ctx?: SeriesCtx): BreakoutSignal {
  if (i < p.k + 1) return NONE;
  const start = Math.max(p.k, i - p.lookback);
  let level = 0;
  for (let j = start; j <= i - 1 - p.k; j++) {          // j + k must be < i (confirmed with past data)
    let isPivot = true;
    for (let m = 1; m <= p.k; m++) {
      if (candles[j].h <= candles[j - m].h || candles[j].h <= candles[j + m].h) { isPivot = false; break; }
    }
    if (isPivot && candles[j].h > level) level = candles[j].h;
  }
  if (level <= 0) return NONE;
  const c = candles[i];
  const av = ctx?.avgVol20[i] ?? avgVolLocal(candles, i);
...
// ── Model C — VCP base breakout (Minervini — the biggest-mover pattern) ──────

export interface VCPParams {
  maxBase: number;        // bars to scan for the base
  maxTightPct: number;    // final-contraction tightness ceiling (%)
  maxDepthPct: number;    // whole-base depth ceiling (%)
  minVolMult: number;     // breakout-bar volume surge
  near52wPct: number;     // base high must be ≥ (1 - near52wPct/100) × 52w high
  requireVolDecline: boolean;
}
function rangeTightness(candles: Candle[], a: number, b: number): number {
  let hi = 0, lo = Infinity, cs = 0, n = 0;
  for (let j = a; j <= b; j++) { if (candles[j].h > hi) hi = candles[j].h; if (candles[j].l < lo) lo = candles[j].l; cs += candles[j].c; n++; }
  const avgC = n > 0 ? cs / n : 0;
  return avgC > 0 ? (hi - lo) / avgC * 100 : Infinity;
}
function avgVolRange(candles: Candle[], a: number, b: number): number {
  let s = 0, n = 0;
  for (let j = a; j <= b; j++) { s += candles[j].v; n++; }
  return n > 0 ? s / n : 0;
}
...
// ── Model D — Pocket pivot (Morales/Kacher — accumulation, earlier entry) ────

export interface PocketParams { emaPeriod: 10 | 50; maxExtPct: number; }
export function modelPocketPivot(candles: Candle[], i: number, p: PocketParams, ctx?: SeriesCtx): BreakoutSignal {
  if (i < 11) return NONE;
  const c = candles[i], prev = candles[i - 1];
  const up = c.c > prev.c;
  let maxDownVol = ctx?.maxDownVol10[i] ?? 0;
  if (!ctx) for (let j = i - 10; j < i; j++) if (candles[j].c < candles[j - 1].c && candles[j].v > maxDownVol) maxDownVol = candles[j].v;
...
// ── Model E — New-high breakout (O'Neil — minimal overhead supply) ───────────

export interface NewHighParams { N: number; volMult: number; maxAbovePct: number; }
export function modelNewHigh(candles: Candle[], i: number, p: NewHighParams, ctx?: SeriesCtx): BreakoutSignal {
  if (i < 1) return NONE;
  const level = ctx?.hh[p.N]?.[i] ?? hhLocal(candles, i, p.N);
  if (level <= 0) return NONE;
  const c = candles[i];
...
// ── Confluence — score how many forces align on bar i ────────────────────────

export interface ConfluenceParams {
  donchian: DonchianParams; swing: SwingParams; vcp: VCPParams;
  pocket: PocketParams; newHigh: NewHighParams;
  minScore: number;
}
export interface ConfluenceResult {
  isBreakout: boolean;
  score: number;                 // 0..1 weighted
  level: number;                 // highest hurdle actually cleared
  fired: Record<string, boolean>;
}
const W = { newHigh: 0.30, vcp: 0.30, pocket: 0.15, swing: 0.15, donchian: 0.10 };
export function confluence(candles: Candle[], i: number, p: ConfluenceParams, ctx?: SeriesCtx): ConfluenceResult {
  ...
  const score =
    (a.isBreakout ? W.donchian : 0) + (b.isBreakout ? W.swing : 0) +
    (c.isBreakout ? W.vcp : 0) + (d.isBreakout ? W.pocket : 0) + (e.isBreakout ? W.newHigh : 0);
  let level = 0;
  for (const s of [a, b, c, d, e]) if (s.isBreakout && s.level > level) level = s.level;
