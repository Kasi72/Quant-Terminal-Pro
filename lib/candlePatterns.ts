...
function body(c: Candle): number { return Math.abs(c.c - c.o); }
function range(c: Candle): number { return c.h - c.l; }
...
function isBull(c: Candle): boolean { return c.c > c.o; }
function isBear(c: Candle): boolean { return c.c < c.o; }
function bodyMid(c: Candle): number { return (c.o + c.c) / 2; }
function bodyTop(c: Candle): number { return Math.max(c.o, c.c); }
function bodyBot(c: Candle): number { return Math.min(c.o, c.c); }
function isDoji(c: Candle): boolean { const r = range(c); return r > 0 && body(c) / r < 0.10; }
function isLargeBody(c: Candle): boolean { const r = range(c); return r > 0 && body(c) / r > 0.60; }
function gapUp(a: Candle, b: Candle): boolean { return b.o > a.h; }
function gapDn(a: Candle, b: Candle): boolean { return b.o < a.l; }

const P = (name: string, short: string, type: 'bullish' | 'bearish' | 'neutral', strength: 1 | 2 | 3): CandlePattern => ({ name, short, type, strength });

export function detectCandlePattern(candles: Candle[], endIdx: number): CandlePattern {
  ...
  const r = range(c);
  if (r < 0.0001) return P('Doji', 'DOJI', 'neutral', 2);
  ...
  // Doji variants
  if (bPct < 0.10) {
    if (lwPct > 0.60 && uwPct < 0.10) return P('Dragonfly Doji', 'DGDF', 'bullish', 2);
    if (uwPct > 0.60 && lwPct < 0.10) return P('Gravestone Doji', 'GRVD', 'bearish', 2);
    if (uwPct > 0.35 && lwPct > 0.35) return P('Rickshaw Man', 'RKMN', 'neutral', 2);
    if (uwPct > 0.30 && lwPct > 0.30) return P('Long-Legged Doji', 'LLDO', 'neutral', 2);
