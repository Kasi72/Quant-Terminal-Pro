...
function computeStatsFeatures(candles, endIdx) {
    ...
    let score = 0;
    if (rsi14val >= 80)
        score += 14; // validated: highest tier, 60.6% WR, +3.24% avg
    else if (rsi14val >= 70)
        score += 9; // 56.2% WR, +2.55% avg
    else if (rsi14val >= 60)
        score += 5; // 54.4% WR — still above baseline
    // RSI14 50-60 is the WORST bucket (49.0% WR, -3.83% avg) — deliberately NOT scored
    if (hurst.trending)
        score += 12; // validated ★★★ — h>0.55 cleanly separates outcomes
    if (cci34val >= 150 && cci34val <= 300)
        score += 6; // validated sweet spot (55-58% WR)
    if (w52.pctFromLow >= 80)
        score += 6; // validated: strong stocks far from 52WL outperform
    if (w52.ddFromHigh <= 10)
        score += 6; // near 52W high = strength (part of winning grid combo)
    if (ttm.squeezeFired && ttm.momentumRising)
        score += 8; // squeeze FIRING is the signal, not squeeze ON alone
    if (volZ.sig)
        score += 5; // weak standalone (-0.031 r) — reduced weight
    else if (volZ.z >= 1.5)
        score += 3;
    if (bb.squeeze)
        score += 6; // weak standalone (+0.017 r) — reduced weight
    else if (bb.pctl <= 20)
        score += 3;
    if (kSqueeze)
        score += 4;
    ...
    if (kSqueeze && bb.squeeze)
        score += 3; // double squeeze bonus
    if (sharpe > 2.5)
        score += 4; // validated bucket threshold (was >1.0)
    if (entropy < 1.5)
        score += 3; // low entropy = tight clustering
    // insideBars>=2 REMOVED — backtest shows it's backward (47.5% WR vs 55.8% at 0 inside bars)
    if (volSkew > 0.2)
        score += 3; // accumulation signal
    if (garch > 1.3)
        score += 2; // vol about to expand
    if (guppy.coiledRelease)
        score += 8; // validated coil-then-release pattern (62.4% WR)
    else if (guppy.cleanBullishFan)
        score += 3; // clean bullish fan alone (55.7% WR)
    return {
