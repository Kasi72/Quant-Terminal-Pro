...
exports.PARAM_SETS = {
    ...
    // 🧪 v11-fp — fingerprint-tuned: bodyPct 5→70, exactVolVsPre5 1.0→3.0 (d=0.54, d=0.50)
    optimized_ultraselective_8plus: {
        name: 'Ultra-Selective v11fp-N500 8+', tag: '82% WR',
        minAvgTurnover20: 10000000, maxATRPct14Pctl120: 30,
...
// booleans
liquidityOk, volOk, zoneOk, pre10RangeOk, pre10ExpOk, pre10VolOk, pre5VolOk, pre10HighVolOk, pre10RedBiasOk, exactRangeOk, exactVolOk, exactVolPre5Ok, closeLocOk, wickOk, bodyOk, riskOk, upsOk, rsi2Ok, volExpOk, cqsOk, volatilityExpansionRatio, candleQualityScore, closeAboveZoneOk, closeAboveZonePct) {
...
function analyzeStock(candles, paramSetKey) {
    ...
    // 18. inflectionScore
    const inflectionScore = computeInflectionScore(zone, params, breakoutOk, pre10AvgRangeATR, pre10AvgVolRatio, pre5AvgVolRatio, pre10RedVolBias, exactRangeATR14, exactVolRatio20, exactVolVsPre5, closeLoc, upperWickPct, bodyPct, signalRangePct, ultraPrecisionScore, rsi2, volatilityExpansionRatio, candleQualityScore);
...
function detectCandleDNA(candles, endIdx, atr14) {
    ...
    if (upperToLowerWickRatio <= 0.5)
        wickCleanliness += 18;
    else if (upperToLowerWickRatio <= 1.0)
        wickCleanliness += 11;
    else if (upperToLowerWickRatio <= 2.0)
        wickCleanliness += 5;
    if (marubozuScore >= 80)
        wickCleanliness += 17;
    else if (marubozuScore >= 70)
        wickCleanliness += 11;
    else if (marubozuScore >= 55)
        wickCleanliness += 5;
    ...
    const tier = score >= 75 ? 'ELITE' : score >= 55 ? 'STRONG' : score >= 35 ? 'GOOD' : 'WEAK';
