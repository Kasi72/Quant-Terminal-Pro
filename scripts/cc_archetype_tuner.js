// ═══════════════════════════════════════════════════════════════════════════════
// CC ARCHETYPE TUNER — sweeps ARCHETYPE_TUNING overrides for CompressionCoil
// analyzeCompressionCoil uses tuned() not PARAM_SETS fields.
//
// Real CC bottlenecks (confirmed from reading analyzeCompressionCoil):
//   1. minGateVolRatio=3  → pre-screen requires signal bar volume ≥ 3× avg
//      (paradox: coiling = dry volume, but gate demands spike)
//   2. minADX=45          → C6 ADX gate during compression phase is almost impossible
//      (compression = low ADX by definition; ADX≥45 = very strong trend)
//   3. minCompressionBars=9 → 9 consecutive sub-ATR bars required
//
// Pass 1: 16-combo grid (minGateVolRatio × minADX) with anchor exit TP=20%/SL=3.5×ATR
...
const { analyzeStock, setArchetypeTuning } = engine;
...
// Grid: real CC bottlenecks via ARCHETYPE_TUNING
// minGateVolRatio: pre-screen vol gate (default=3, paradoxically high for compression)
// minADX: C6 ADX requirement (default=45, too high for consolidation phase)
const VOL_RATIO_VALUES = [1.0, 1.3, 1.5, 2.0];   // minGateVolRatio
const MIN_ADX_VALUES   = [20, 28, 35, 45];        // minADX
...
// ── Load universe ─────────────────────────────────────────────────────────────
const totalCombos = VOL_RATIO_VALUES.length * MIN_ADX_VALUES.length;
console.log('═══════════════════════════════════════════════════════════════════════════════');
console.log('  CC ARCHETYPE TUNER  |  OOS cutoff: 2024-01-01');
console.log(`  Grid: ${VOL_RATIO_VALUES.length} minGateVolRatio × ${MIN_ADX_VALUES.length} minADX = ${totalCombos} combos`);
console.log('  Bottlenecks: minGateVolRatio=3 (vol spike paradox) + minADX=45 (too high for compression)');
console.log('═══════════════════════════════════════════════════════════════════════════════\n');
...
function collectSignals(tuningOverrides) {
  setArchetypeTuning(CC_KEY, tuningOverrides);
  const signals = [];
  ...
  setArchetypeTuning(CC_KEY, null);
  return signals;
}
...
console.log('  NOTE: Current defaults: minGateVolRatio=3 (vol spike), minADX=45 (compression paradox)');
console.log('  Hypothesis: lowering both dramatically increases signal count with maintained quality.\n');

const pass1Results = [];
let comboIdx = 0;

for (const volRatio of VOL_RATIO_VALUES) {
  for (const minADX of MIN_ADX_VALUES) {
    comboIdx++;
    const tuningOverrides = { minGateVolRatio: volRatio, minADX };
    const signals = collectSignals(tuningOverrides);
    const oos = signals.filter(s => s.isOos);
    const { n, wr, pf, avgPnl } = anchorPF(signals, ANCHOR_TP, ANCHOR_SL);
    pass1Results.push({ volRatio, minADX, nOOS: oos.length, nTotal: signals.length, n, wr, pf, avgPnl, signals });
    process.stdout.write(`  Combo ${String(comboIdx).padStart(2)}/${totalCombos}: volRatio=${volRatio.toFixed(1)} ADX≥${String(minADX).padStart(2)} → OOS=${oos.length} WR=${wr.toFixed(0)}% PF=${pf.toFixed(2)}\n`);
  }
}

console.log('\n─── Top results (sorted by OOS n, then PF) ───');
console.log('  volRat | ADX≥ | n_OOS | WR%    | PF    | AvgPnL%');
console.log('  ───────┼──────┼───────┼────────┼───────┼────────');
...
for (const r of sorted.slice(0, 10)) {
  ...
  console.log(`  ${r.volRatio.toFixed(1).padStart(6)} | ${String(r.minADX).padStart(4)} | ${String(r.nOOS).padStart(5)} | ${wr}% | ${pf} | ${avg}%`);
}
...
for (const combo of topCombos) {
  const { volRatio, minADX, nOOS, signals } = combo;
  ...
  console.log(`─── CC: volRatio≥${volRatio} ADX≥${minADX} (OOS n=${nOOS}) ───`);
  ...
  if (profitable.length > 0) {
    ...
    console.log(`\n  ★ CHAMPION: volRatio≥${volRatio} ADX≥${minADX} TP=${best.tpPct}% SL=${best.slMult}×ATR`);
    ...
    if (!bestChampion || best.pf > bestChampion.pf) {
      bestChampion = { volRatio, minADX, ...best };
    }
  } else {
...
// ── Final summary ─────────────────────────────────────────────────────────────
console.log('═══ CC ARCHETYPE TUNER SUMMARY ═══\n');
if (bestChampion) {
  console.log(`✅ CC CHAMPION FOUND`);
  console.log(`   minGateVolRatio: ${bestChampion.volRatio}  (was: 3.0)`);
  console.log(`   minADX: ${bestChampion.minADX}  (was: 45)`);
  ...
  console.log(`   ACTION: Apply to stockEngine.ts analyzeCompressionCoil() tuned() defaults:`);
  console.log(`     tuned(key, 'minGateVolRatio', ${bestChampion.volRatio})`);
  console.log(`     tuned(key, 'minADX', ${bestChampion.minADX})`);
  console.log(`   AND update ARCHETYPE_EXIT_DEFAULTS + remove CC from WATCHLIST_ONLY_PARAM_SETS`);
} else {
  const bestByN = sorted[0];
  console.log(`❌ NO CHAMPION — max OOS: n=${bestByN?.nOOS} (need ≥12 with PF>1.2, WR>50%, wins≥8)`);
  console.log(`   CC remains WATCHLIST_ONLY`);
  console.log(`   Best: volRatio≥${bestByN?.volRatio} ADX≥${bestByN?.minADX} → OOS=${bestByN?.nOOS} WR=${bestByN?.wr.toFixed(1)}% PF=${bestByN?.pf.toFixed(2)}`);
}
console.log('\n═══ CC ARCHETYPE TUNER COMPLETE ═══\n');
