...
function threeBarSwingLow(candles, idx) {
    ...
    for (let i = 0; i < candlesSinceEntry.length; i++) {
        ...
        if (stopBreached) {
            ...
            if (t1InRange) {
                ...
                // Three conditions: RSI-2 < 10 + close within 0.25×ATR of stop + buyer evidence.
                // Fixes APLLTD (close 0.29×ATR below stop) and GOODLUCK (close 0.68×ATR below stop).
                if (!blocked) {
        ...
        if (t1Hit && close > highestCloseSinceT1)
            highestCloseSinceT1 = close;
