...
function computeATR14(candles, idx) {
    if (idx < 1)
        return candles[0] ? candles[0].h - candles[0].l : 1;
...
// ─── EMA helper ──────────────────────────────────────────────────────────────
function computeEMAAt(candles, idx, period) {
    if (idx < period - 1)
        return 0;
...
function candleDate(candle, fallbackBase, dayIndex) {
    if (candle.d && candle.d.length >= 10)
        return candle.d.slice(0, 10);
    if (candle.ts)
        return new Date((candle.ts + 19800) * 1000).toISOString().slice(0, 10);
...
function validateTrade(trade, candlesSinceEntry, options = {}) {
    ...
    if (!trade || !Number.isFinite(trade.entryPrice) || trade.entryPrice <= 0)
        return defaultResult;
    if (!Array.isArray(candlesSinceEntry) || candlesSinceEntry.length === 0)
        return defaultResult;
    ...
    const hardStop = validDisasterStop && (!validReviewStop || trade.disasterStop < reviewStop)
        ? trade.disasterStop
        : reviewStop;
    const riskPerShare = (0, tradeOps_1.getTradeRiskPerShare)(trade);
    ...
    for (let i = 0; i < monitoredCandles.length; i++) {
        ...
        if (!candle || !Number.isFinite(candle.h) || !Number.isFinite(candle.l))
            continue;
        ...
        if (t2Hit && close > highestCloseSinceT2)
            highestCloseSinceT2 = close;
    }
...
function computeRollingStats(trades, lastN, label) {
    const closed = trades.filter(tradeOps_1.isTradeResolvedForWinRate).slice(-lastN);
    const wins = closed.filter(tradeOps_1.didReachFivePctTarget);
    const losses = closed.filter(t => !(0, tradeOps_1.didReachFivePctTarget)(t));
    const maeTrades = closed.filter(t => (0, tradeOps_1.getTradeMaePct)(t) > 0);
    return {
        period: label,
        total: closed.length,
        wins: wins.length,
        ...
        avgMFE: wins.length > 0 ? wins.reduce((s, t) => s + (0, tradeOps_1.getTradeMfePct)(t), 0) / wins.length : 0,
        avgMAE: maeTrades.length > 0 ? maeTrades.reduce((s, t) => s + (0, tradeOps_1.getTradeMaePct)(t), 0) / maeTrades.length : 0,
