...
exports.computeStatsFeatures = computeStatsFeatures;
exports.computeExpectedValue = computeExpectedValue;
exports.computeKelly = computeKelly;
exports.computeVaR95 = computeVaR95;
exports.computeBayesianProb = computeBayesianProb;
const candlePatterns_1 = require("./candlePatterns");
// ─── Helpers ─────────────────────────────────────────────────────────────────
function mean(arr) {
    if (arr.length === 0)
        return 0;
...
function computeBBSqueeze(candles, endIdx) {
    const period = 20;
    if (endIdx < period)
        return { width: 0, pctl: 50, squeeze: false };
    const closes = [];
    ...
    for (let e = histStart; e < endIdx; e++) {
        ...
        const d = stdDev(cl);
        if (s > 0)
            widthHistory.push((s + 2 * d - (s - 2 * d)) / s);
    }
...
function compute52W(candles, endIdx) {
    ...
    for (let i = start; i <= endIdx; i++) {
        if (candles[i].h > high52)
            high52 = candles[i].h;
        if (candles[i].l < low52)
            low52 = candles[i].l;
    }
...
function computeEntropy10(candles, endIdx) {
    ...
    const range = max - min;
    if (range < 0.001)
        return 0; // perfectly flat = zero entropy
    const bins = 5;
...
function computeGARCH(candles, endIdx) {
    ...
    for (let i = 10; i < returns.length; i++) {
        ...
        if (sigma2 < 1e-10)
            sigma2 = 1e-10; // variance floor to prevent collapse to 0
    }
...
function computeTTMSqueeze(candles, endIdx) {
    ...
    // Bollinger Bands
    const bbCloses = [];
    for (let i = endIdx - bbPeriod + 1; i <= endIdx; i++)
        bbCloses.push(candles[i].c);
    ...
    if (endIdx > bbPeriod + 1) {
        const prevBBCloses = [];
        for (let i = endIdx - bbPeriod; i < endIdx; i++)
            prevBBCloses.push(candles[i].c);
...
function computeGuppySpread(candles, endIdx) {
    ...
    function checkSpringAt(idx) {
        ...
        if (minS <= maxL)
            return false; // C3: bull coil (early exit)
        ...
        if (!(vol20 > 0 && candles[idx].v < vol20))
            return false; // C4
        if (!(s.spreadPct < sPrev.spreadPct && sPrev.spreadPct < sPrev2.spreadPct))
            return false; // C5: streak≥2
...
function computeStatsFeatures(candles, endIdx) {
    ...
    // Candle pattern
    const candlePat = (0, candlePatterns_1.detectCandlePattern)(candles, endIdx);
    // EMA/SMA levels
    ...
    let score = 0;
    if (rsi14val >= 80)
        score += 14; // validated: 80-85→+3.55%, 85-90→+10.56% avg20d
    else if (rsi14val >= 70)
        score += 9; // 70-75→+1.47%, 75-80→+1.43%
    else if (rsi14val >= 60)
        score += 5; // 60-65→+1.79%, 65-70→+1.25%
    // Hurst trending REMOVED — r=0.000 (zero correlation), was wasting 12pts
    if (cci34val >= 100 && cci34val <= 200)
        score += 6; // narrowed: 150-200→+2.35% (sweet spot confirmed)
    if (w52.pctFromLow >= 80 && w52.pctFromLow <= 400)
        score += 6; // validated: far from 52WL outperforms; cap at 400% to exclude penny-stock recoveries
    // ddFromHigh REMOVED — r=−0.037 (NEGATIVE). Near 52WH = exhaustion, not strength, in breakout context.
    if (ttm.squeezeFired && ttm.momentumRising)
        score += 8; // squeeze firing confirmed: +0.011 r
    if (volZ.sig)
        score -= 5; // INVERTED: blow-off volume r=−0.074 (strongly NEGATIVE)
    else if (volZ.z >= 1.5)
        score -= 2; // moderate blow-off penalty
    if (bb.squeeze)
        score += 6; // confirmed positive: r=+0.012
    else if (bb.pctl <= 20)
        score += 3; // confirmed: tight BB width → continuation
    if (kSqueeze)
        score += 4;
    if (lr.flat)
        score += 4;
    ...
    if (skew.positive)
        score += 4;
    if (kSqueeze && bb.squeeze)
        score += 3; // double squeeze bonus: r=+0.012
    if (sharpe > 2.5)
        score += 4;
    if (entropy < 1.5)
        score += 3;
    ...
    if (garch > 1.3)
        score += 2;
    if (guppy.coiledRelease)
        score += 12; // INCREASED: r=+0.030, +3.60% avg20d, 62% win rate
    else if (guppy.cleanBullishFan)
        score += 3; // confirmed: r=+0.030, +1.80% avg20d
    return {
...
function computeExpectedValue(winRate, avgWinPct, avgLossPct) {
    if (avgLossPct >= 0)
        return 0; // loss must be negative; mirror computeKelly guard
    const wr = winRate / 100;
    return wr * avgWinPct + (1 - wr) * avgLossPct;
}
// ─── #7: Kelly Criterion ─────────────────────────────────────────────────────
function computeKelly(winRate, avgWinPct, avgLossPct, fraction = 0.25) {
    const wr = winRate / 100;
    const lr = 1 - wr;
    if (lr <= 0)
        return 25; // 100% win rate → max Kelly fraction
    if (avgLossPct >= 0)
        return 0; // no loss data or wrongly signed
...
function computeBayesianProb(candles, endIdx) {
    ...
    for (let i = Math.max(1, endIdx - lookback); i < endIdx - 5; i++) {
        ...
        if (Math.abs(cl - closeLoc) < 0.15 && Math.abs(bp - bodyPct) < 0.15) {
            ...
            const futureHigh = Math.max(...Array.from({ length: 5 }, (_, j) => i + j + 1 <= endIdx ? candles[i + j + 1].h : 0));
            if (futureHigh >= c.c * 1.05)
                wins++;
        }
    }
