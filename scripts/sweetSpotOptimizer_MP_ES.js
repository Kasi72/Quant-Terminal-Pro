...
const MIN_OOS    = 50; // minimum OOS trades to qualify
...
function collectWorker() {
  ...
  for (const file of workerData.files) {
    ...
    processed++;
    if (c.length < WINDOW + 21) { short++; continue; } // require at least 21 outcome bars
    usable++;
...
// Expanded sweet-spot search space grid generators
function sweetSpotGridGen(id, k) {
  if (id === 'MomentumPocket') {
    return {
      ...
      minCloseVsEMA20: gv([-999, -2, -1, 0, 0.5], k, 47), // includes disabled option -999!
      minEMA20VsEMA50: gv([-999, 0, 0.5, 1.0], k, 53), // includes disabled option -999!
      ...
      tpPct: 5, // Locked at +5% Profit Target
  ...
  if (id === 'EMAStack') {
    return {
      ...
      minCloseVsEMA20: gv([-999, -1, -0.5, 0, 0.5], k, 23), // includes optional gates
...
function evaluate(events, id, p) {
  const ti = 0; // fixed TP = 5% is first element
  ...
  for (const a of Object.values(by)) {
    ...
    for (const e of a) {
      ...
      nextIdx = exit + 1; // enforce non-overlap
    }
  }
...
function fitness(r) {
  ...
  // Custom sweet-spot objective function: Weighting OOS PF and hit5 heavily
  const pf = x => Math.min(8, Math.max(0, x.pf)) / 8; // scale PF up to 8.0
  const avg = x => Math.max(0, Math.min(5, x.avg + 1)) / 6;
  const sampleBonus = Math.min(0.05, Math.log10(r.oos.n) / 100);

  // High conviction hit rate: P(Hit 5% before stop)
...
async function main() {
  ...
  console.log(`\nSWEET-SPOT HYPER-OPTIMIZER — Sweeping ${files.length} stocks using ${workers} workers...`);
  console.log(`Optimizing FOR: +5% target | OOS cutoff: ${OOS_CUT} | Combos: ${COMBOS}\n`);
  ...
  const jp = path.join(OUT_DIR, `sweet_spot_opt_${stamp}.json`);
