...
// Hyper-tunes all 6 Momentum Archetype sets via IS/OOS backtesting
// Architecture: Workers precompute binary outcomes for a fixed exit-param grid,
//   then main thread sweeps 10k random detection-param combos per set.
...
function simOutcomes(candles, signalIdx, atr, entryOpen) {
  // Returns Int8Array of length N_EXIT: 1=win, -1=loss, 0=no trade
  const outcomes = new Int8Array(N_EXIT); // all 0 by default
  ...
  for (let tpI = 0; tpI < TP_G.length; tpI++) {
    ...
    for (let slI = 0; slI < SL_G.length; slI++) {
      ...
      for (let hbI = 0; hbI < HB_G.length; hbI++) {
        ...
        let result = 0; // 0 = timeout (handled below)
        let lastClose = entryOpen;
        for (let j = signalIdx + 1; j < Math.min(n, signalIdx + 1 + maxBars); j++) {
          lastClose = candles[j].c;
          if (candles[j].h >= tp && candles[j].l <= sl) { result = -1; break; } // both: conservative=loss
          if (candles[j].h >= tp) { result = 1; break; }
          if (candles[j].l <= sl) { result = -1; break; }
        }
        if (result === 0) result = lastClose > entryOpen ? 1 : -1; // timeout
    ...
    for (let i = 60; i < n - 1; i++) {
      ...
      if ((!setId || setId === 3) && turnover20 >= 10_000_000) {
        ...
        if (dd52W >= 5 && dd52W <= 65) {
          ...
          ev3.push({ isOOS, outcomes,
            dd: dd52W, sb: stabBars, cl: closeLoc, bp: bodyPct, ig: isGreen, vr: volRatio20, r14: rsi14,
            dp: diP, dm: diM, adx: adxV, bsc: bscV, dib: diBull });
        }
      }
      ...
      if (!setId || setId === 5) {
        ...
        if (fires >= 1) {
          ...
          ev5.push({ isOOS, outcomes, vf: vfF, cc: ccF, mp: mpF, es: esF, dmi: diBull, adx: adxV, fires });
        }
      }

      // ── Set 6: ORS-Prime ─────────────────────────────────────────────────
      if ((!setId || setId === 6) && !isNaN(ema20[i])) {
...
function sweepSet(label, events, filterFn, genFn, nCombos, minOosN) {
  ...
  for (let c = 0; c < nCombos; c++) {
    ...
    let isW=0, isN=0, oosW=0, oosN=0;
    for (const e of isEvt) {
      if (!filterFn(e, p)) continue;
      const o = e.outcomes[ei];
      if (o === 0) continue;
      isN++; if (o > 0) isW++;
    }
    if (isN < 8) continue;
    ...
    results.push({ isN, isWR: isW/isN, oosN, oosWR: oosW/oosN, wil: wilsonLower(oosW, oosN), p });
  }

  results.sort((a, b) => b.wil - a.wil);
  ...
  console.log(`Rank  IS_n  IS_WR%  OOS_n  OOS_WR%  Wil%  Detection params  |  Exit params`);
  console.log('─'.repeat(110));
  for (let i = 0; i < top20.length; i++) {
    ...
    console.log(
      `${String(i+1).padStart(4)}  ${String(r.isN).padStart(4)}   ${(r.isWR*100).toFixed(1).padStart(4)}%  ` +
      `${String(r.oosN).padStart(5)}    ${(r.oosWR*100).toFixed(1).padStart(4)}%  ${(r.wil*100).toFixed(1).padStart(4)}%  ` +
  ...
  const best = top20[0];
  console.log(`\n★ SWEET SPOT — ${label}`);
  console.log(`  IS : n=${best.isN}  WR=${(best.isWR*100).toFixed(1)}%`);
  console.log(`  OOS: n=${best.oosN}  WR=${(best.oosWR*100).toFixed(1)}%  Wilson=${(best.wil*100).toFixed(1)}%`);
...
function genVF() { return { minVolRatio: +rand(1.2,6.0).toFixed(1), minCloseLoc: +rand(40,92).toFixed(0), maxNearHi20Pct: +rand(2,22).toFixed(0), minRangeATR: +rand(0.6,3.5).toFixed(1), maxGapDownPct: +rand(-6,0).toFixed(1), ...dmiGen(), tpPct: pickTP(), slAtrMult: pickSL(), maxHoldBars: pickHB() }; }
function filtVF(e,p) { return e.vr>=p.minVolRatio && e.cl>=p.minCloseLoc && e.nh<=p.maxNearHi20Pct && e.ra>=p.minRangeATR && e.gd>=p.maxGapDownPct && dmiPass(e,p); }

function genCC() { return { minComprBars: randInt(1,8), maxComprBars: randInt(3,15), minVolDecDays: randInt(0,4), minPricePos20: +rand(45,88).toFixed(0), maxBBWidthPctl: +rand(15,60).toFixed(0), maxRangeATR: +rand(0.5,2.2).toFixed(1), ...dmiGen(), tpPct: pickTP(), slAtrMult: pickSL(), maxHoldBars: pickHB() }; }
function filtCC(e,p) { return e.cb>=p.minComprBars && e.cb<=p.maxComprBars && e.vd>=p.minVolDecDays && e.pp>=p.minPricePos20 && e.bb<=p.maxBBWidthPctl && e.ra<=p.maxRangeATR && dmiPass(e,p); }

function genMP() { return { minDd52W: +rand(5,32).toFixed(0), maxDd52W: +rand(28,62).toFixed(0), minStabBars: randInt(0,6), minCloseLoc: +rand(35,78).toFixed(0), minBodyPct: +rand(20,65).toFixed(0), requireGreen: Math.random()>0.4?1:0, minVolRatio: +rand(0.8,3.2).toFixed(1), minRSI14: +rand(18,48).toFixed(0), maxRSI14: +rand(48,75).toFixed(0), ...dmiGen(), tpPct: pickTP(), slAtrMult: pickSL(), maxHoldBars: pickHB() }; }
function filtMP(e,p) { return e.dd>=p.minDd52W && e.dd<=p.maxDd52W && e.sb>=p.minStabBars && e.cl>=p.minCloseLoc && e.bp>=p.minBodyPct && (!p.requireGreen||e.ig) && e.vr>=p.minVolRatio && e.r14>=p.minRSI14 && e.r14<=p.maxRSI14 && dmiPass(e,p); }

function genES() { return { minBelowBars: randInt(1,14), minEMA10VsEma20: +rand(-6,3).toFixed(1), minVolRatio: +rand(1.0,4.2).toFixed(1), maxRSI2Last5: +rand(12,58).toFixed(0), todayCrossOnly: Math.random()>0.5?1:0, ...dmiGen(), tpPct: pickTP(), slAtrMult: pickSL(), maxHoldBars: pickHB() }; }
function filtES(e,p) { return e.bc>=p.minBelowBars && e.e10>=p.minEMA10VsEma20 && e.vr>=p.minVolRatio && e.mr<=p.maxRSI2Last5 && (!p.todayCrossOnly||e.ct) && dmiPass(e,p); }

function genPS() { return { minFires: randInt(2,4), requireDMI: Math.random()>0.5?1:0, minADX: ADX_OPTS[randInt(0,ADX_OPTS.length-1)], tpPct: pickTP(), slAtrMult: pickSL(), maxHoldBars: pickHB() }; }
function filtPS(e,p) { return e.fires>=p.minFires && (!p.requireDMI||e.dmi) && (p.minADX===0||e.adx>=p.minADX); }
...
async function main() {
  ...
  const setDefs = [
    ...
    { name:'Set6 — ORS-Prime v3',             ev: all.ev6, gen: genORS, filt: filtORS },
  ];

  for (const s of setDefs) {
    console.log(`\n${'─'.repeat(80)}`);
    console.log(`Sweeping ${N_COMBOS.toLocaleString()} combos — ${s.name}`);
    sweepSet(s.name, s.ev, s.filt, s.gen, N_COMBOS, MIN_OOS_N);
  }
